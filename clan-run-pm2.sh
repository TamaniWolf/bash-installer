...
while [ $choice -eq 4 ]; do
    ...
    if [[ $choice -eq 1 ]] ; then
        ...
        cd "$root/ClanBot"
        sudo pnpm update
        pm2 start clanbot.js --name "$name" --max-memory-restart 250M && echo "Done" && exit 0
        echo "Done"
        ...
        cd "$root/ClanBot"
        sudo pnpm update
        pm2 start clanbot.js --name "$name" --max-memory-restart 250M && pm2 save && echo "Done" && exit 0
        echo "Done"
